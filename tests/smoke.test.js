...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Jewellery")
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/6b91356f-d7c7-598c-9081-59fee1195ec8')
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/ebd922f2-6e8f-55bf-89e3-929ab1494983')
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain("Tunisia")
    expect(host.textContent).toContain('Higher Institute for the Study of Contemporary History of Tunisia, University of Manouba')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/42fb6391-e1f6-527a-85db-f9638b02d667')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Higher Institute for the Study of Contemporary History of Tunisia, University of Manouba'), { timeout: 20000 })
    expect(host.textContent).toContain("Tunis")
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/42fb6391-e1f6-527a-85db-f9638b02d667/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['jewellery', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
